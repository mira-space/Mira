...
class TCrossAttention(CrossAttention):
    def T(self):
        return None
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 relative_position=False, temporal_length=None, use_image_dataset=False, lora_temporal=False):
        super().__init__()
        ...
        if not use_linear:
            ...
            self.proj_in = nn.Linear(in_channels, inner_dim)
        if lora_temporal:
            UseCrossAttention = TCrossAttention
        else:
            UseCrossAttention = CrossAttention
        if relative_position:
            assert(temporal_length is not None)
            attention_cls = partial(UseCrossAttention, relative_position=True, temporal_length=temporal_length)
        else:
            attention_cls = partial(UseCrossAttention, relative_position=False, temporal_length=temporal_length)
