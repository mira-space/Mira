...
from .samplers.sampler_utils import get_nested_attribute
...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pretrain:
            print("### Load VAE Pretrain: {}".format(pretrain))
            try:
                st = torch.load(pretrain, map_location='cpu')
            except:
                from safetensors import safe_open
                st = {}
                with safe_open(pretrain, framework="pt", device="cpu") as f:
                    for key in f.keys():
