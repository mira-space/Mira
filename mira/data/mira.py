...
class Mira(Dataset):
    ...
    def __init__(self,
                 ...
                 fps_cond=False,
                 max_framestride=8,
                 Webvid_meta=None,webvid_prob=0.5,dense_meta=None
                 ):
        self.meta_path = meta_path
        self.data_dir = data_dir
        self.Webvid_meta = Webvid_meta
        self.dense_meta = dense_meta
    ...
    def _load_metadata(self):
        ...
        self.dense_metadata = {}
        if self.dense_meta:
            dense_metadata = pd.read_csv(self.dense_meta)
            if self.subsample is not None:
                dense_metadata = dense_metadata.sample(self.subsample, random_state=0)
            self.dense_metadata = dense_metadata
            self.dense_metadata.dropna(inplace=True)


    def _get_video_path(self, sample):
        rel_video_fp = sample['Path']
        # full_video_fp = os.path.join(self.data_dir, 'videos', rel_video_fp)
        return rel_video_fp, rel_video_fp
    ...
    def __getitem__(self, index):
        ...
        while True:
            if random.random() > self.webvid_prob:
                if index % len(self) < len(self.metadata):
                    ...
                    sample = self.dense_metadata.iloc[index]
                    video_path, rel_fp = self._get_video_path(sample)
                    caption = sample['Dense caption']
            else:
    ...
    def __len__(self):
        return  len(self.metadata) + len(self.dense_metadata)
