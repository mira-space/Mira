...
def load_model_checkpoint(model, ckpt):
    ...
    elif 'ema' in pl_sd.keys():
        model.model.diffusion_model.load_state_dict(pl_sd["ema"], strict=True)
    else:
        # deepspeed
...
def save_results(prompt, samples, inputs, filename, realdir, fakedir, fps=10, upscale=True):
    ## save prompt
    ...
    for idx, video in enumerate(videos):
        ...
        if upscale:
            os.environ['PATH'] = '/data/miniconda3/envs/env-i2v/bin/:' + os.environ['PATH']
            try:
                os.chdir('extralibs/Practical-RIFE/')
            except:
                pass
            os.system('sudo python inference_video.py --video={} --multi=4 > /dev/null 2>&1'.format(path))
            os.system('sudo python inference_video_enhance.py --video={} > /dev/null 2>&1'.format(
                path[:-4] + '_4X_24fps.mp4'))


def inference_prompt(model, prompts, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                     unconditional_guidance_scale=1.0, unconditional_guidance_scale_temporal=None,
                     negative_prompts=None, **kwargs):
    ...
    for _ in range(n_samples):
        ...
        #     samples = interpolate(samples,  mode='nearest-exact', size=(int(1.5*t), h, w))
        # x_T = model.q_sample(x_T, torch.full((x_T.shape[0],), 999, device=x_T.device, dtype=torch.long))
        # ddim_sampler=   DDIMSampler(model)
        # noise_shape = x_T.shape
        # samples, _ = ddim_sampler.sample(S=ddim_steps,
        #                                  conditioning=batch_cond,
        #                                  batch_size=noise_shape[0],
        #                                  shape=noise_shape[1:],
        ...
        #                                  eta=ddim_eta,
        #                                  temporal_length=noise_shape[2],
        #                                  conditional_guidance_scale_temporal=unconditional_guidance_scale_temporal,x_T=x_T,
        #                                  **kwargs
        #                                  # guidance_rescale=0.7, ddim_discretize="uniform_trailing", **kwargs
        #                                  )
...
def run_inference(args, rank, gpu_num):
    ...
    # model_config['params']['denoiser_config']['params']['temporal_length']= 30
    ...
    if args.prompt_file:
        ...
        while True:
            ...
            for i in tqdm(range(0, len(indices), args.bs), desc='Sample Batch'):
                ...
                batch_samples = inference_prompt(model, prompts, noise_shape, args.n_samples, args.ddim_steps,
                                                 args.ddim_eta, \
                                                 args.unconditional_guidance_scale,
                                                 args.unconditional_guidance_scale_temporal,
                                                 cond_tau=args.cond_tau, target_size=args.target_size,
                                                 fps=[args.fps] * args.bs, dilated_conv=args.dilated_conv,
                                                 dilated_conv_tau=args.dilated_conv_tau, free_u=args.free_u,
            ...
            def decode(z, mini_b=10):
                ...
                if mini_b is None:
                    ...
                    for i in range(t // mini_b):
                        torch.cuda.empty_cache()
                        curr = decoder.decode(z[i], **kwargs)
                        results.append(curr.unsqueeze(0))
            ...
            while all_sample:
                ...
                if batch_samples.dim() == 5:
                    ...
                    while (batch_samples_cpu):
                        ...
                        filename = "%04d_Version%04d_Iter%04d" % (nn, i, iter)
                        save_results(prompt, res, None, filename, realdir, fakedir, fps=6)
                        i += 1
                        del res
                nn += 1
...
def get_parser():
    ...
    parser.add_argument("--upscale", type=bool, default=True, help="")
