...
sys.path.append("../..")
sys.path.append("..")
from mira.data.base import Txt2ImgIterableBaseDataset
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
                 ...
                 test_max_n_samples=None, iv_ratio=0.5,iterloader=False,prefetch_factor=2,linear_ratio=False):
        super().__init__()
        ...
        if train_img is not None:
            ...
            if train is None:
                self.train_dataloader=img_data.train_dataloader
        else:
    ...
    def _train_dataloader(self):
        ...
        loader = DataLoader(self.datasets["train"], batch_size=self.batch_size,
                          ...
                          worker_init_fn=init_fn, collate_fn=self.collate_fn, prefetch_factor=self.prefetch_factor
                          )
        if self.img_loader is not None:
            if self.iterloader:
                iv_ratio = self.iv_ratio
                self.iter1 = iter(loader)
                self.iter2 = iter(self.img_loader)
                return self.alternating_dataloader(loader, self.img_loader, p1=iv_ratio)
            else:
                return {"loader_video": loader, "loader_img": self.img_loader}
        else:
            return loader
        
    def alternating_dataloader(self, dataloader1, dataloader2, p1=0.5):
        while True:
            if self.linear_ratio:
                self.cur_iter += 1
                p1 = min(self.cur_iter/30000,1)
            if random.random() > p1:
                try:
                    yield next(self.iter1)
                except StopIteration:
                    print('runout iter1')
                    self.iter1 = iter(dataloader1)
            else:
                try:
                    yield next(self.iter2) 
                except StopIteration:
                    print('runout iter2')
